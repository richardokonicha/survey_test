...
import { SearchProps, ClickRatingEvent, Target } from "../react-app-env";
      ...
      <div className="flex items-center sm:space-x-3 space-x-1 font-xs">
        {['1', '2', '3', '4', '5'].map((item: string, index: number) => (
          <button
            ...
            className={`text-primary bg-white rounded-full flex justify-center items-center w-7 h-7 font-xs ${ratingFilter.includes(item) && 'bg-accent text-slate-50 '
              } hover:shadow-lg`}
          >
            {item}
          </button>
